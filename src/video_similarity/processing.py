...
def extract_aggregated_features(
    ...
    try:
        ...
        scene_manager.detect_scenes(
            video=video, show_progress=True, frame_skip=frame_skip
        )
        ...
        for scene_idx, scene in enumerate(scene_list):
            ...
            if descriptors is not None and len(descriptors) > 0:
                ...
                logger.debug(
                    f"Scene {scene_idx}: extracted {len(descriptors)} descriptors"
                )
            else:
...
def _compare_scene_features(
    scenes_a: List[np.ndarray], scenes_b: List[np.ndarray]
) -> float:
    """
    ...
    scene_weights = [
        len(scene) / total_descriptors_a for scene in scenes_a if len(scene) > 0
    ]
    ...
    weighted_score = sum(
        score * weight for score, weight in zip(scene_matches, scene_weights)
    )

    # Apply density compensation factor
    density_ratio = min(total_descriptors_a, total_descriptors_b) / max(
        total_descriptors_a, total_descriptors_b
    )
